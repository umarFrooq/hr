...
const createNew = async (schema, body) => {
    return await schema.create(body).then(result => {
        return { status: 200, isSuccess: true, data: result, message: "Created successfully." }
    }).catch(err => {
        throw new ApiError(400, err.message);
        // return { status: 400, isSuccess: false, data: err, message: "Creation unsuccessfull." }
    });
}
...
const find = async (schema, filter, options, pagination = true) => {
    if (pagination) {
        return await schema.paginate(filter, options).then(result => {
            return { status: 200, isSuccess: true, data: result }
        }).catch(err => {
            return { status: 400, isSuccess: false, data: err }
        });
    } else if (!pagination) {
...
const deleteById = async (schema, id) => {
    return await schema.findByIdAndDelete(id).then(result => {
        return { status: 200, isSuccess: true, data: result, message: "Deleted successfully" }
    }).catch(err => {
        return { status: 400, isSuccess: false, data: {}, message: err }
    });
}
 ...
 * @param {object} body - body
  */
const updateById = async (schema, _id, body) => {
    return await schema.findByIdAndUpdate(_id, body, { returnNewDocument: true }).then(result => {
        return { status: 200, isSuccess: true, data: result, message: "Updated successfuly." }
    }).catch(err => {
        return { status: 400, isSuccess: false, data: {}, message: err }
    });
}
...
const findById = async (schema, id) => {
    return await schema.findById(id).then(result => {
        if (!result)
            return { status: 404, isSuccess: false, data: {}, message: "not found" }
        return { status: 200, isSuccess: true, data: result ? result : {}, message: "" }
    }).catch(err => {
        return { status: 400, isSuccess: false, data: err, message: "Server error." }
    });
}
...
const findOne = async (schema, filter) => {
    return await schema.findOne(filter).then(result => {
        return { status: 200, isSuccess: true, data: result ? result : null, message: "" };
    }).catch(err => {
        return { status: 400, isSuccess: false, data: err, message: "Server error." };
    });
}
...
const createMany = async (schema, dataArray) => {
    ...
    else return { status: 400, isSuccess: false, data: [], message: "Porduct length is 0" }
}
...
const findOneAndPopulate = async (schema, filter, populate, pagination = false, options, select) => {
    ...
    return schema.aggregate(query).then(result => {
        ...
        return { status: 200, isSuccess: true, data: response && Object.keys(response).length ? response : null, message: "" };
    }).catch(err => {
        return { status: 400, isSuccess: false, data: err, message: "Server error." };
    })
    // }
...
const deleteMany = async (schema, filter) => {
    return schema.deleteMany(filter).then(result => {
        return responseMethod(200, true, "Deleted sucessfully", result && result.length ? result : null)
    }).catch(err => {
        return responseMethod(400, false, null, err);
    })
}
...
const paginationParser = (options) => {
    ...
    if (options) {
        ...
        if (options.sortBy) {
            console.log(typeof options.sortBy);
            if (typeof options.sortBy == "object")
                optionQuery.sort = options.sortBy;
        }
...
const aggregationPagination = (schema, searchQuery, options, facetQuery) => {
    ...
    return schema.aggregate(query).then(result => {
        if (!result.length)
            return responseMethod(200, true, "", {
                page: pagination.page ? pagination.page : 1,
                limit: pagination.limit ? pagination.limit : 10,
                totalPages: 0, totalResult: 0, results: []
            });
        return responseMethod(200, true, "", result && result.length ? result[0] : [])
    }).catch(err => {
