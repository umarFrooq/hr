...
const paginate = (schema) => {
  ...
  schema.statics.paginate = async function (filter, options) {
    ...
    const populate = options && options.populate ? options.populate : null;
    ...
    else if (select && !options.lean && !options.populate)
      docsPromise = this.find(filter).select(select).sort(sort).skip(skip).limit(limit).exec();

    else if(select && !options.lean && options.populate)
      docsPromise = this.find(filter).select(select).populate(populate).sort(sort).skip(skip).limit(limit).exec();
    else if(!select && options.populate)
      docsPromise = this.find(filter).populate(populate).sort(sort).skip(skip).limit(limit).exec();
